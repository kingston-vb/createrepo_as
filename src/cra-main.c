...
main (int argc, char **argv)
{
	...
	ctx = cra_context_new ();
	cra_context_set_no_net (ctx, no_net);
	cra_context_set_api_version (ctx, api_version);
	cra_context_set_add_cache_id (ctx, add_cache_id);
	cra_context_set_extra_checks (ctx, extra_checks);
	cra_context_set_use_package_cache (ctx, use_package_cache);
	cra_context_set_old_metadata (ctx, old_metadata);
	cra_context_set_extra_appstream (ctx, extra_appstream);
	cra_context_set_extra_appdata (ctx, extra_appdata);
	cra_context_set_extra_screenshots (ctx, extra_screenshots);
	cra_context_set_screenshot_uri (ctx, screenshot_uri);
	...
	for (i = 0; i < packages->len; i++) {
		...
		if (cra_context_find_in_cache (ctx, filename)) {
	...
	cra_context_disable_older_pkgs (ctx);

	/* process all packages in the pool */
	ret = cra_context_process (ctx, &error);
	if (!ret) {
